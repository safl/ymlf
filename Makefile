DOC_BUILD_DIR=build
PROJECT_NAME=ymlf
PROJECT_VERSION_MAJOR=$(shell grep "VERSION_MAJOR = ." bin/ymlf | cut -d " " -f 3)
PROJECT_VERSION_MINOR=$(shell grep "VERSION_MINOR = ." bin/ymlf | cut -d " " -f 3)
PROJECT_VERSION_PATCH=$(shell grep "VERSION_PATCH = ." bin/ymlf | cut -d " " -f 3)
